...
class UserCreateView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'users/create.html', {'is_session_active': is_session_active, 'form': form})
...
class UserDeleteView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return handle_error(request, 'У вас нет прав для изменения другого пользователя.', 'users_index')

    def post(self, request, *args, **kwargs):
        user_id = kwargs.get('pk')
        user = User.objects.get(id=user_id)
        if not services.check_user(Tasks, user):
            return handle_error(request, 'Невозможно удалить пользователя, потому что он используется', 'users_index')
        user.delete()
        logout(request)
        return handle_success(request,'Пользователь успешно удален', 'users_index')
